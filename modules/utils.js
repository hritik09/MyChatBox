...
exports.roomExists = function(req, res, client, fn) {
  client.hget('mychatbox:rooms:keys', encodeURIComponent(req.body.room_name), function(err, roomKey) {
    if(!err && roomKey) {
...
exports.createRoom = function(req, res, client) {
  ...
  client.hmset('rooms:' + roomKey + ':info', room, function(err, ok) {
    if(!err && ok) {
      client.hset('mychatbox:rooms:keys', encodeURIComponent(req.body.room_name), roomKey);
      client.sadd('mychatbox:public:rooms', roomKey);
      res.redirect('/' + roomKey);
    } else {
      res.send(500);
    }
...
exports.getPublicRoomsInfo = function(client, fn) {
  client.smembers('mychatbox:public:rooms', function(err, publicRooms) {
    var rooms = []
...
exports.getPublicRooms = function(client, fn){
  client.smembers("mychatbox:public:rooms", function(err, rooms) {
